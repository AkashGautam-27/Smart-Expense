...
function generateStaticAdvisorInsights(transactions: any[], budgets: any[], budgetStatuses: any[]) {
  const lowBudgets = (budgetStatuses || [])
    .filter((b: any) => b.percentage >= 75)
    .map((b: any) => ({
      category: b.category,
  ...
  const savingTips = [
    "Review recurring subscriptions under Shopping or Entertainment to cut back on unused services.",
    "Consider allocating at least 15% to 20% of your monthly net income to savings directly upon receipt.",
    "Track your payment channels. Opt for Cash or UPI budget caps if Card payments lead to impulse purchases."
  ];
...
Ensure your advice is action-oriented, precise, uses real numbers from the data, and avoids vague or dry template text. Do not output anything except the JSON object.`;
    ...
    // Generate high-quality rule-based fallback insights
    const fallbackData = generateStaticAdvisorInsights(transactions || [], budgets || [], budgetStatuses || []);
    res.json({ 
      data: fallbackData, 
      warning: 'The Gemini service is temporarily rate-limited. Displaying local rule-based financial insights.' 
    });
  }
});
