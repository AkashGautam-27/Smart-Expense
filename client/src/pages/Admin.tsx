...
export default function Admin() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || 'An error occurred while loading users.';
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Failed to register new user.';
      setError(errorMessage);
    } finally {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.message || 'Failed to delete user account.';
      setError(errorMessage);
    } finally {
      setFormLoading(false);
    }
  ...
  return (
    <div id="admin_panel" className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold text-slate-900 dark:text-neutral-100 flex items-center gap-2.5">
