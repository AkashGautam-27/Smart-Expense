...
export default function Dashboard() {
  ...
  // 2. Format Data for Category Donut Chart
  const donutData = useMemo(() => {
    const today = new Date();
    const currentYear = today.getFullYear();
    const currentMonth = today.getMonth();

    const monthlyExpenses = activeTransactions.filter(t => {
      if (t.type !== 'expense') return false;
      const txDate = new Date(t.date + 'T00:00:00');
      return txDate.getFullYear() === currentYear && txDate.getMonth() === currentMonth;
    });
  ...
  // 3. Format Date / Months for Cash Flow Bar Chart
  const cashFlowData = useMemo(() => {
    // Generate Income vs Expense data for May and June
    const months = ['May', 'June'];
    const years = [2026, 2026];

    return months.map((month, idx) => {
      const targetMonthIdx = month === 'May' ? 4 : 5; // 0-indexed May/June
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[11px] text-slate-400 dark:text-neutral-500 mt-1 font-medium">Earned this month</span>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[11px] text-slate-400 dark:text-neutral-500 mt-1 font-medium">Spent in current period</span>
          </div>
        </motion.div>
          ...
          <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-neutral-800/50">
            <div>
              ...
              <p className="text-xs text-slate-400 dark:text-neutral-500 mt-0.5">
                May vs June side-by-side volume distribution
              </p>
            </div>
                ...
                donutData.slice(0, 4).map((d, idx) => (
                  <div key={d.name} className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-400 dark:text-neutral-500 font-bold">
                      {metrics.expense > 0 ? `${((d.value / metrics.expense) * 100).toFixed(0)}%` : '0%'}
                    </span>
                  </div>
                ))
              ) : (
        ...
        <div className="col-span-1 md:col-span-2 lg:col-span-3 bg-white dark:bg-neutral-900 p-6 rounded-2xl border border-slate-200 dark:border-neutral-800/70 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-1">
            {budgetStatuses.slice(0, 6).map(b => (
                  ...
                  <>
                    ...
                    {!aiError && aiConfidence !== 0 && (
                      <div className="flex justify-between items-center text-[10px] font-semibold text-slate-400 bg-slate-50 dark:bg-neutral-950 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-neutral-800">
                        <span>Gemini extraction parsing accuracy:</span>
